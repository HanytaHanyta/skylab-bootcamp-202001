...
describe.only('maxgamers', () => {
    ...
    it('should succeed on valid retrieve escaperoom by query', async () => {
        const retrievedEscapeRooms = await maxgamers(query)
        ...
        retrievedEscapeRooms.forEach(element => {
            ...
            expect(element.maxplayers).to.exist
            expect(element.maxplayers).to.be.a('number')
            expect(element.maxplayers).to.equal(query)
